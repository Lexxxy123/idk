    ...
    public List<PetAbility> getPetAbilities(SItem instance) {
        ...
        ArrayList<PetAbility> abilities = new ArrayList<PetAbility>(Collections.singletonList(new PetAbility(){
            ...
            public void onDamage(EntityDamageByEntityEvent e2) {
                ...
                double c2 = 0.3 * (double)level / 100.0;
                if (r2 < d2) {
        ...
        if (instance.getRarity().isAtLeast(Rarity.EPIC)) {
            abilities.add(new PetAbility(){
                ...
                public void onHurt(EntityDamageByEntityEvent e2, Entity damager) {
                    double d2;
                    double r2 = SUtil.random(0.0, 1.0);
                    double c2 = 0.1 * (double)level / 100.0;
                    if (r2 < d2) {
                        BigDecimal dmgabs = new BigDecimal(e2.getDamage()).setScale(1, RoundingMode.HALF_EVEN);
                        ((Player)e2.getEntity()).playSound(((Player)e2.getEntity()).getLocation(), Sound.ITEM_BREAK, 1.0f, 1.0f);
                        ((Player)e2.getEntity()).sendMessage(Sputnik.trans("&eTank of the Motherland ability has absorbed &c" + SUtil.commaify(dmgabs.doubleValue()) + " &edamage for you!"));
                        e2.setDamage(0.0);
                    }
                }
            });
