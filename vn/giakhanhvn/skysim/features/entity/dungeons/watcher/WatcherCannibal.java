...
public class WatcherCannibal
extends BaseZombie {
    @Override
    public String getEntityName() {
        return Sputnik.trans("&4&lMaster Cannibal");
    }
    ...
    public void onSpawn(final LivingEntity entity, SEntity sEntity) {
        HeadsOnWall h2 = new HeadsOnWall(EnumWatcherType.CANNIBAL);
    ...
    public SEntityEquipment getEntityEquipment() {
        return new SEntityEquipment(SItem.of(SMaterial.GOLD_SWORD).getStack(), null, null, SItem.of(SMaterial.CHAINMAIL_LEGGINGS).getStack(), SItem.of(SMaterial.LEATHER_BOOTS).getStack());
    }
    ...
    public void onDamage(SEntity sEntity, Entity damager, EntityDamageByEntityEvent e2, AtomicDouble damage) {
        ...
        SUtil.delay(() -> WatcherCannibal.lambda$onDamage$0((Entity)en, v2), 1L);
    }

    @Override
    public void onAttack(EntityDamageByEntityEvent e2) {
        ((LivingEntity)e2.getDamager()).setHealth(Math.min(((LivingEntity)e2.getDamager()).getMaxHealth(), ((LivingEntity)e2.getDamager()).getHealth() + ((LivingEntity)e2.getDamager()).getMaxHealth() * 5.0 / 100.0));
    }
