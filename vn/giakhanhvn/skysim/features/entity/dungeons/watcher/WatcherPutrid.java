...
extends BaseZombie {
    ...
    public void onSpawn(final LivingEntity entity, SEntity sEntity) {
        HeadsOnWall h2 = new HeadsOnWall(EnumWatcherType.PUTRID);
    ...
    public void onDamage(SEntity sEntity, Entity damager, EntityDamageByEntityEvent e2, AtomicDouble damage) {
        ...
        SUtil.delay(() -> WatcherPutrid.lambda$onDamage$0((Entity)en, v2), 1L);
    }

    @Override
    public void onAttack(EntityDamageByEntityEvent e2) {
        LivingEntity killed = (LivingEntity)e2.getEntity();
        if (SUtil.random(0, 20) == 1) {
            ...
            killed.getNearbyEntities(5.0, 5.0, 5.0).forEach(entity -> {
                if (entity instanceof Player) {
                    ...
                    p2.damage(1.0E-5);
                    User.getUser(p2.getUniqueId()).damage(p2.getMaxHealth() * 10.0 / 100.0, EntityDamageEvent.DamageCause.ENTITY_ATTACK, (Entity)killed);
                }
            });
        }
    }
