...
public class WatcherReaper
extends BaseZombie {
    @Override
    public String getEntityName() {
        return Sputnik.trans("&4&lMaster Reaper");
    }
    ...
    public void onSpawn(final LivingEntity entity, SEntity sEntity) {
        HeadsOnWall h2 = new HeadsOnWall(EnumWatcherType.REAPER);
    ...
    public SEntityEquipment getEntityEquipment() {
        return new SEntityEquipment(SItem.of(SMaterial.STONE_SWORD).getStack(), null, SItem.of(SMaterial.CHAINMAIL_CHESTPLATE).getStack(), SItem.of(SMaterial.IRON_LEGGINGS).getStack(), SItem.of(SMaterial.LEATHER_BOOTS).getStack());
    }
    ...
    public void onDamage(SEntity sEntity, Entity damager, EntityDamageByEntityEvent e2, AtomicDouble damage) {
        ...
        SUtil.delay(() -> WatcherReaper.lambda$onDamage$0((Entity)en, v2), 1L);
    }

    @Override
    public void onAttack(EntityDamageByEntityEvent e2) {
        if (e2.getEntity() instanceof Player && ((LivingEntity)e2.getEntity()).getHealth() - e2.getFinalDamage() <= 0.0) {
            ((LivingEntity)e2.getDamager()).setHealth(((LivingEntity)e2.getDamager()).getMaxHealth());
        }
    }
