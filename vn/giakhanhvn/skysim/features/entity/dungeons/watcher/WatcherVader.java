...
public class WatcherVader
extends BaseZombie {
    @Override
    public String getEntityName() {
        return Sputnik.trans("&4&lMaster Vader");
    }
    ...
    public void onSpawn(final LivingEntity entity, SEntity sEntity) {
        HeadsOnWall h2 = new HeadsOnWall(EnumWatcherType.VADER);
    ...
    public SEntityEquipment getEntityEquipment() {
        return new SEntityEquipment(SItem.of(SMaterial.GOLD_AXE).getStack(), null, SItem.of(SMaterial.IRON_CHESTPLATE).getStack(), SItem.of(SMaterial.CHAINMAIL_LEGGINGS).getStack(), SItem.of(SMaterial.CHAINMAIL_BOOTS).getStack());
    }
    ...
    public void onDamage(SEntity sEntity, Entity damager, EntityDamageByEntityEvent e2, AtomicDouble damage) {
        ...
        SUtil.delay(() -> WatcherVader.lambda$onDamage$0((Entity)en, v2), 1L);
    }

    @Override
    public void onAttack(EntityDamageByEntityEvent e2) {
        if (e2.getEntity() instanceof Player) {
            Player p2 = (Player)e2.getEntity();
            if (SUtil.random(0, 3) == 1) {
                User.getUser(p2.getUniqueId()).damage(p2.getMaxHealth() * 5.0 / 100.0);
            }
        }
    }
