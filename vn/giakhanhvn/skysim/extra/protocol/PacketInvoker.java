...
public class PacketInvoker {
        ...
        new BukkitRunnable(){
            ...
            public void run() {
                ...
                for (Entity e2 : drop.getNearbyEntities(0.07, 0.07, 0.07)) {
                    if (!(e2 instanceof Player) || (Player)e2 != owner) continue;
    ...
    public static void dropVoidSpawner(final Player owner, Location loc) {
        for (Entity e2 : owner.getWorld().getEntities()) {
            if (!e2.hasMetadata("owner") || !((MetadataValue)e2.getMetadata("owner").get(0)).asString().equals(owner.getUniqueId().toString())) continue;
            return;
        }
        ...
        new BukkitRunnable(){
            ...
            public void run() {
                ...
                for (Entity e2 : drop.getNearbyEntities(0.07, 0.07, 0.07)) {
                    if (!(e2 instanceof Player) || (Player)e2 != owner || !((Player)e2).isSneaking()) continue;
