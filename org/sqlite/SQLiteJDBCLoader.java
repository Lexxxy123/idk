    ...
    private static boolean loadNativeLibrary(String path, String name) {
        ...
        if (libPath.exists()) {
            try {
                ...
                logger.error("Failed to load native library: {}. osinfo: {}", name, OSInfo.getNativeLibFolderPathForCurrentOS(), e2);
                return false;
            }
        }
        return false;
    ...
    public static final class VersionHolder {
        ...
        static {
            ...
            try {
                ...
                LoggerFactory.getLogger(VersionHolder.class).error("Could not read version from file: {}", (Object)versionFile, (Object)e2);
            }
            VERSION = version;
        }
    }
