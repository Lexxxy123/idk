...
public class JSONObject {
    ...
    public JSONObject(JSONTokener x2) throws JSONException {
        ...
        block8: while (true) {
            ...
            String key = x2.nextSimpleValue(c2).toString();
            c2 = x2.nextClean();
            if (c2 != ':') {
    ...
    public JSONObject(Map<?, ?> m2) {
        this(m2, 0, new JSONParserConfiguration());
    }

    public JSONObject(Map<?, ?> m2, JSONParserConfiguration jsonParserConfiguration) {
        this(m2, 0, jsonParserConfiguration);
    }

    private JSONObject(Map<?, ?> m2, int recursionDepth, JSONParserConfiguration jsonParserConfiguration) {
        ...
        if (m2 == null) {
            ...
            for (Map.Entry<?, ?> e2 : m2.entrySet()) {
                ...
                this.map.put(String.valueOf(e2.getKey()), JSONObject.wrap(value, recursionDepth + 1, jsonParserConfiguration));
            }
        }
    }
    ...
    private static <A extends Annotation> A getAnnotation(Method m2, Class<A> annotationClass) {
        ...
        if (c2.getSuperclass().equals(Object.class)) {
            return null;
        }
        try {
        ...
        if (initial >= '0' && initial <= '9' || initial == '-') {
            ...
            if (JSONObject.isDecimalNotation(val)) {
                try {
                    ...
                    if (initial == '-' && BigDecimal.ZERO.compareTo(bd2) == 0) {
                        return -0.0;
                    }
                    return bd2;
            ...
            if ((bi2 = new BigInteger(val)).bitLength() <= 31) {
                return bi2.intValue();
            }
            if (bi2.bitLength() <= 63) {
    ...
    private static String removeLeadingZerosOfNumber(String value) {
        ...
        int n2 = counter = negativeFirstChar ? 1 : 0;
