...
public class XML {
    ...
    private static void removeEmpty(JSONObject jsonObject, XMLParserConfiguration config) {
        if (jsonObject.has(config.getcDataTagName())) {
            Object s2 = jsonObject.get(config.getcDataTagName());
            if (s2 instanceof String) {
                if (XML.isStringAllWhiteSpace(s2.toString())) {
                    jsonObject.remove(config.getcDataTagName());
                }
            } else if (s2 instanceof JSONArray) {
                JSONArray sArray = (JSONArray)s2;
                for (int k2 = sArray.length() - 1; k2 >= 0; --k2) {
                    String s1;
                    Object eachString = sArray.get(k2);
    ...
    private static boolean isStringAllWhiteSpace(String s2) {
        for (int k2 = 0; k2 < s2.length(); ++k2) {
            char eachChar = s2.charAt(k2);
    ...
    public static JSONObject toJSONObject(Reader reader, XMLParserConfiguration config) throws JSONException {
        ...
        XMLTokener x2 = new XMLTokener(reader, config);
        while (x2.more()) {
            x2.skipPast("<");
            if (!x2.more()) continue;
